...
const TRIP_NAME = "Lakewood, WA → Astoria, OR";
const DAY_MILES = 10;
...
// distance along the resulting path. Ends at Megler, the WA foot of the
// Astoria–Megler Bridge — the bridge bans pedestrians, so the Columbia crossing
// to Astoria is a shuttle/ride (see CROSSING below), then walking resumes.
...
// The Columbia River crossing. No legal pedestrian route: the Astoria–Megler
// Bridge prohibits foot traffic (4.1 mi, no walkway) and the nearest ferry is
// ~50 mi upriver at Puget Island. Modelled as an assisted hop.
const CROSSING = {
  from: [46.256, -123.884], // Megler, WA
  to: [46.1879, -123.8313], // Astoria, OR
  label: "Astoria–Megler Bridge",
  note: "No pedestrian access — cross by bike/pedestrian shuttle or a ride (~4 mi).",
};
...
async function main() {
  ...
  const body =
    ...
    ` * DETAIL: ${waypoints.length - 1} walking days (~${DAY_MILES} mi), ${TRIP_NAME},\n` +
    ` * routed with Valhalla pedestrian costing (legal on-foot paths).\n` +
    ` * CROSSING: the Astoria–Megler Bridge (no pedestrian access) — assisted hop.\n` +
    ...
    `  },\n` +
    `];\n\n` +
    `export const CROSSING = ${JSON.stringify(CROSSING)};\n\n` +
